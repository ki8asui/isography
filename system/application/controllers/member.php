...
class Member extends Base_Controller {
	...
	function Member()
	{
		...
		if(empty($this->subdomain)) exit('This controller is only for subdomains.'); // This controller is only for subdomains.
		if(!$this->isOwnerOfSite() && !$this->isMemberOfSite() && $this->uri->segment(2) != 'create') exit('You cannot access this controller.'); // Only members and owner can access this controller.						    				
	...
	function create()
	{
		...
		$_data['site_name'] = $sitename = $this->site_name;

		if(!$this->subdomainId || ((empty($firstname) || empty($lastname)) && !empty($ask_name)) )
		{
		...
		else
		{
			...
			$this->User_is_member_of_site_model->insert($userId, $this->subdomainId, $subscribe_y_n = NULL, $facebook_pics_y_n = NULL, $facebook_vids_y_n = NULL, $member_y_n = NULL, $facebook_infinite_key = NULL);

			// sending thanks for applying message
			$this->_sendemail('apply',array('sitename'=>$sitename,'username'=>$firstname,'email'=>$email));
		}
		...
		{
			$this->load->view('member_registration_success', $_data);
		}
	...
	function index()
	{
		...
		*/
		...
		if(!empty($page)) $page = (int)$page; else $page = 1;
		if($page < 0) $page = 1;
		$offset = ($page - 1) * $limit;
		...
		if(!empty($_POST))
		{
			...
			$_data['searchtext'] = $searchtext = $this->input->post('searchtext');
		...
		if(!empty($searchtext))
		{
			$where .= " and message like '%" . mysql_escape_string($searchtext) . "%'";
			$_data['form'] = 'sform';
		}
		...
		$_data['posts'] = $posts = $this->Post_model->getPosts($limit, $offset, $where, $order = 'posts.id DESC');
		...
		*/
		...
		$_data['offset'] = $offset;
		$_data['total_rows'] = $totalRows;
		$_data['per_page'] = $limit;
		$_data['total_pages'] = ceil($totalRows / $limit);
		$_data['page'] = $page;

		$this->load->view('member_top', $_data);
	...
	function ajax_getposts()
	{
		...
		*/
		...
		$_data['posts'] = $posts = $this->Post_model->getPosts($limit = 10, $offset, $where, $order = 'posts.id DESC');
		$totalRows = $this->Post_model->getListCount($where);
		...
		$this->load->view('ajax/member_posts', $_data);
	...
	function uploadpicture()
	{
		...
		else
		{
			...
			//$albumQuery = new Zend_Gdata_Photos_UserQuery();
			$albumQuery->setImgMax(200);
			...
			if ($insertedEntry->getMediaGroup()->getContent() != null)
			{
				...
				$contentUrl = $mediaContentArray[0]->getUrl();
		              //print"<pre>";var_dump($mediaContentArray);print"</pre>";
	...
	function admin($action='',$count=0,$sort='',$ord='')
	{
		...
		$this->load->view('member_top', $_data);
                ...
                $adata['sites'] = $this->Site_model->getWhere(array('owner_id' => $this->getUserId()));
		$this->load->view('owner_top', $adata);
		...
		if ($action=='appQueue') //---first tab. Application Queue
		{
			if ($this->input->post('action'))
			{
				...
				//---send approval email
					...
					$sitedata=$this->Site_model->getById($this->subdomainId);
					$this->_sendemail('approval',array('username'=>$userdata['username'],'sitename'=>$sitedata['name'],'subdomain'=>$sitedata['subdomain'],'email'=>$userdata['email']));
				}
