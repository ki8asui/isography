...
<?elseif($post['post_type'] == 'video'):?>
	<?php foreach($post['videos'] as $video):?>
		...
		<div style="float: left; margin: 0px 0px 10px 30px; padding: 4px; border: solid 1px #666666; background: #ffffff;"><?php echo preg_replace('/.+?v=(.+)/','<object width="340" height="285"><param name="movie" value="http://www.youtube.com/v/${1}&hl=en&fs=1&rel=0&border=0"></param><param name="allowFullScreen" value="true"></param><param name="allowscriptaccess" value="always"></param><embed src="http://www.youtube.com/v/${1}&hl=en&fs=1&rel=0&border=0" type="application/x-shockwave-flash" allowscriptaccess="always" allowfullscreen="true" width="340" height="285"></embed></object>',$video['vid_url']);?></div>
	<?php endforeach;?>
<?elseif($post['post_type'] == 'picture'):?>
	<?php foreach($post['pictures'] as $picture):?>
		...
		<a target="_blank" href="<?php echo preg_replace('/s200/','s640',$picture['pic_url']);?>"><img src="<?=$picture['pic_url']?>" style="padding: 4px; margin: 10px; background: #ffffff; border: solid 1px #666666; float: left;"/></a>
	<?php endforeach;?>
<?endif;?>
</p>
